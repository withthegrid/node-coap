...
function CoAPServer(options, listener) {
  ...
  let maxAge = parameters.exchangeLifetime * 1000;
 ...
 */
CoAPServer.prototype._handle = function(serverRequest) {
  const { packet, rsinfo } = serverRequest;
  ...
  response = new Message(packet, function(response, responsePacket) {
    ...
    var key = that._toKey(request, responsePacket, responsePacket.ack || !responsePacket.confirmable)
    lru.set(key, buf)
    buf.sender = sender

    if (that._options.sendAcksForNonConfirmablePackets || responsePacket.confirmable){
  ...
  response._cachekey = this._toCacheKey(request)
  ...
  if(block1Buff) {
    ...
    if(blockState) {
      if(request.payload.length == 0 && !blockState.moreBlocks) {
        debug("Cleaning up the packet. Received a Block1 message option but that's already been received");
...
CoAPServer.prototype._toCacheKey = function (request) {
  if (request._packet.token && request._packet.token.length > 0) {
    return request._packet.token.toString('hex') + '/' + this._clientIdentifier(request)
  }

  return null
}

CoAPServer.prototype._toKey = function (request, packet, appendToken) {
